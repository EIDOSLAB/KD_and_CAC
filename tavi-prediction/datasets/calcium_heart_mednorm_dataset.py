...
# All scans are resized to (151,236,269) (D*H*W) ->
...
# ct are then normalized using normalizations as described in Medicalnet paper MED3D: TRANSFER LEARNING FOR 3D MEDICAL IMAGE ANALYSIS
...
ct_cache = get_cache('calcium_mednorm_ct')
...
def get_ct(data_path: str, segmentation_model_path: str, device: str, patient: str):
    ...
    ct = Ct(f'{data_path}/{patient["id"]}/ct/', file_pattern=f'{patient["files_pattern"]}*.dcm', clip=False)
    sorted_voxels = ct.img.view(-1).sort().values
    min_position = int(round(len(sorted_voxels)/1000*5))
    max_position = len(sorted_voxels)-min_position
    min_clip = sorted_voxels[min_position]
    max_clip = sorted_voxels[max_position]
    
    ct.img[convex_hull==0] = min_clip
    
    ret_img = torch.full((151,236,269), min_clip)
    
    ct.crop(z=(props[0]['bbox'][0], props[0]['bbox'][3]),
            y=(props[0]['bbox'][1], props[0]['bbox'][4]),
            x=(props[0]['bbox'][2], props[0]['bbox'][5]))

    # scale to fit median spacing (1.25, 0.693359, 0.693359)
    median_spacing = (1.25, 0.693359, 0.693359)
    ct_spacing = ct.metadata.get_voxel_size()
    ct.scale(size=(round(ct.img.shape[0] * (ct_spacing[0] / median_spacing[0])),
                   round(ct.img.shape[1] * (ct_spacing[1] / median_spacing[1])),
                   round(ct.img.shape[2] * (ct_spacing[2] / median_spacing[2]))))
    
    #log.info(f'{patient["id"]}: scaled shape={ct.img.shape}')
    
    copy_image_in_the_middle(ct.img, ret_img)
    
    ret_img = torch.clip(ret_img, min_clip, max_clip)
    norm = T.Normalize(sorted_voxels.mean(), sorted_voxels.std())

    return norm(ret_img)


class CalciumHeartMednormDataset(Dataset):
    ...
    def get_cac_score(self, db, patient: str):
        db.execute(f'SELECT cac_score FROM {CalciumHeartMednormDataset.PATIENT_TABLE} WHERE id = \'{patient}\';')
        rows = db.fetchall()
        if len(rows) != 1:
